...
void setup() {      //The main function of the code, loads files, plays the song, and outputs to the Arduino
  ...
  File playlist = new File("C:\\Users\\CJT19\\Desktop\\ProjectCodenonBlue");      //Makes a new file of the directory of MP3's
  minim = new Minim(this);                                        //Calls Minim so that it can load files from the data directory
  arduino = new Arduino(this, "COM5", 57600);                     //Creates a new arduino, this equals the parent name, COM3 is the port, and 57600 is the connection speed
  
  for(File f : playlist.listFiles()){ 
          String g = f.getName();                              //Iterates through the directory of files and gets those files names
     song = minim.loadFile(g,2048);  
     System.out.println(f.getName());     //Loads the next file, f.getName() is the file name and 2048 is the buffer size
     time = 0;                                     //Sets the time counter to zero
     song.play();                                  //Plays the song
     time = millis();                              //Starts counting the time again
     ...
     beat.setSensitivity(10);                                          //Sets the sensitivity to 10 milliseconds, after a beat has been detected, the algorithm will wait for 10 milliseconds before
...
void draw() {
  ...
  if(beat.isKick()) arduino.analogWrite(ledPin, 255);       //Sends the value of RGB (0, 120, 0) to the Arduino
                                                                               //lights up green
  if(beat.isSnare()) arduino.analogWrite(ledPin2, 255);     //Sends the value of RGB (255, 0, 0) to the Arduino
                                                                               //lights up red
  if(beat.isHat()) arduino.analogWrite(ledPin3, 255);       //Sends the value of RGB (0, 0, 200) to the Arduino
               //lights up blue
 if (keyPressed) {
    state = !state;
...
/*void keyPressed() {
  if (key == 'p' && !state) { //failed test on pausing
    state = true;
    song.pause();
  } else if (key == 'p' && state) {
    state = false;
...
/*void skipKey(){                                                                          //Function that skips to the next song

  String skipKeyPress = String.valueOf(key);                                             //String that keeps track of the button you press
  
  if(song.isPlaying() && skipKeyPress.equals("s")) song.skip(song.length() - time);      //Skips the song if you press "n"
  
}

void rewindKey(){                                                        //Function that rewinds the song
  
  String rewindKeyPress = String.valueOf(key);                           //String that keeps track of the button you press
  
  if(song.isPlaying() && rewindKeyPress.equals("r")) song.rewind();      //Rewinds song if you press "r"
