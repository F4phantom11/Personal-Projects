...
void calc_speed(dungeon *d){
 int speed=10;
   for(int h=0;h<12;h++){
     
     if(d->PC->equip[h]){
       speed+= d->PC->equip[h]->get_speed();
  }
...
void io_display(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(23, 30, "PC Mana: %d ",d->PC->Mana);
  mvprintw(23, 45, "PC Cooldown Heal: %d ",d->PC->cdH);
   mvprintw(22, 25, "PC Cooldown Spell: %d ",d->PC->cdS);
...
void io_display_inv(dungeon *d){
  int i;
 
   for(i=0;i<10;i++){
    
     if(d->PC->invt[i]){
       mvprintw(i + 6, 40, "%d: %s",i,d->PC->invt[i]->get_name());
     }
     else{
       mvprintw(i + 6, 40, "%d: (empty)",i);
        }
...
void io_display_equip(dungeon *d){
  ...
  for(i=0;i<12;i++){
    char t= i+97;
     if(d->PC->equip[i]){
       mvprintw(i + 6, 40, "%c: %s",t,d->PC->equip[i]->get_name());
     }
     else{
       mvprintw(i + 6, 40, "%c: (empty)",t);
        }
   ...
   char t = ' ';
    while(t != 27){
      ...
      int slot = t-48;
        object* tmp;
      if(d->PC->invt[slot]){
       if(d->PC->invt[slot]->get_type()== objtype_WEAPON){
	 tmp =  d->PC->equip[weapon];
  d->PC->equip[weapon] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[weapon]->get_name(),d->PC->equipC);
   
   
     }
   else   if(d->PC->invt[slot]->get_type()== objtype_OFFHAND){
 tmp =  d->PC->equip[offH];
  d->PC->equip[offH] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[offH]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_RANGED){
   tmp =  d->PC->equip[range];
  d->PC->equip[range] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[range]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_LIGHT){
   tmp =  d->PC->equip[light];
  d->PC->equip[light] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[light]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_HELMET){
   tmp =  d->PC->equip[helm];
  d->PC->equip[helm] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[helm]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_CLOAK){
   tmp =  d->PC->equip[cloak];
  d->PC->equip[cloak] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[cloak]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_GLOVES){
   tmp =  d->PC->equip[gloves];
  d->PC->equip[gloves] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[gloves]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_ARMOR){
   tmp =  d->PC->equip[arm];
  d->PC->equip[arm] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[arm]->get_name(),d->PC->equipC);
      ...
      else   if(d->PC->invt[slot]->get_type()== objtype_RING &&d->PC->equip[ring1]==NULL){
   tmp =  d->PC->equip[ring1];
  d->PC->equip[ring1] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[ring1]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_RING){
   tmp =  d->PC->equip[ring2];
  d->PC->equip[ring2] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
  io_queue_message("GOT ITEM %s,%d",d->PC->equip[ring2]->get_name(),d->PC->equipC);
   
   
     }
      else   if(d->PC->invt[slot]->get_type()== objtype_BOOTS){
   tmp =  d->PC->equip[boots];
  d->PC->equip[boots] = d->PC->invt[slot];
  d->PC->invt[slot]= tmp;
...
void io_drop(dungeon *d){
   ...
   char t = ' ';
    while(t != 27){
      ...
      int slot = t-48;
       if(d->PC->invt[slot]){
      objpair(d->PC->position)=d->PC->invt[slot];
      d->PC->invt[slot]=NULL;
      d->PC->invC--;
      calc_speed(d);
       }
...
void io_take(dungeon *d){
 int i;

   for(i=0;i<12;i++){
    
     if(d->PC->equip[i]){
       mvprintw(i + 6, 40, "%c: %s",i+97,d->PC->equip[i]->get_name());
     }
     else{
       mvprintw(i + 6, 40, "%c: (empty)",i+97);
        }
   ...
   char t = ' ';
    while(t != 27){
	 ...
	 for(i=0;i<10;i++){
	   if(d->PC->invt[i]==NULL){
	  d->PC->invt[i]=  d->PC->equip[slot];
	 d->PC->equip[slot]=NULL;
...
void io_inspect(dungeon *d){
   ...
   char t = ' ';
    while(t != 27){
      ...
      if(t-48>=0&&t-48<=9){
      int slot = t-48;
      if(d->PC->invt[slot]){clear();
	 mvprintw(i + 6, 0, "%s",d->PC->invt[slot]->get_desc().c_str());
       }
      }
        for(i=0;i<10;i++){
...
void io_mondesc(dungeon *d){
  ...
  * not otherwise used.                                                 */
  ...
  if(charpair(dest)){
    clear();
    refresh();
    mvprintw(18, 0, "%s",((npc *)charpair(dest))->get_descs());
  }
}
char io_tochar(pair_t dest,dungeon *d){
   switch (mappair(dest)) {
    case ter_wall:
    case ter_wall_immutable:
    case ter_unknown:
...
void io_range(dungeon *d){
  ...
  refresh();
  count++;
    } while (!select(STDIN_FILENO + 1, &readfs, NULL, NULL, &tv)&&count!=2000);
    /* Can simply draw the terrain when we move the cursor away, *
     * because if it is a character or object, the refresh       *
     * function will fix it for us.                              */
  ...
  * not otherwise used.                                                 */
    ...
    case 'y':
    case KEY_HOME:
      if (dest[dim_y] != 1&&d->PC->position[dim_y]-2<dest[dim_y]&&d->PC->known_terrain[dest[dim_y]-1][dest[dim_x]]!=ter_wall) {
        dest[dim_y]--;
      }
      if (dest[dim_x] != 1&& d->PC->position[dim_x]-2<dest[dim_x]&&d->PC->known_terrain[dest[dim_y]][dest[dim_x]-1]!=ter_wall) {
        dest[dim_x]--;
      }
      break;
    case '8':
    case 'k':
    case KEY_UP:
      if (dest[dim_y] != 1&& d->PC->position[dim_y]-3<dest[dim_y]&&d->PC->known_terrain[dest[dim_y]-1][dest[dim_x]]!=ter_wall) {
        dest[dim_y]--;
      }
      break;
    case '9':
    ...
    case KEY_PPAGE:
      ...
      if (dest[dim_x] != DUNGEON_X - 2&& d->PC->position[dim_x]+2>dest[dim_x] &&d->PC->known_terrain[dest[dim_y]][dest[dim_y]+1]!=ter_wall) {
        dest[dim_x]++;
      }
      break;
    case '6':
    case 'l':
    case KEY_RIGHT:
      if (dest[dim_x] != DUNGEON_X - 2&& d->PC->position[dim_x]+3>dest[dim_x]&&d->PC->known_terrain[dest[dim_y]][dest[dim_x]+1]!=ter_wall) {
        dest[dim_x]++;
      }
      break;
    case '3':
    case 'n':
    case KEY_NPAGE:
      if (dest[dim_y] != DUNGEON_Y - 2&&d->PC->position[dim_y]+2>dest[dim_y]&&d->PC->known_terrain[dest[dim_x]][dest[dim_y]+1]!=ter_wall) {
        dest[dim_y]++;
      }
      if (dest[dim_x] != DUNGEON_X - 2&& d->PC->position[dim_x]+2>dest[dim_x]&&d->PC->known_terrain[dest[dim_y]][dest[dim_x]+1]!=ter_wall) {
        dest[dim_x]++;
      }
      break;
    case '2':
    case 'j':
    case KEY_DOWN:
      if (dest[dim_y] != DUNGEON_Y - 2&& d->PC->position[dim_y]+3>dest[dim_y]&&d->PC->known_terrain[dest[dim_x]][dest[dim_y]+1]!=ter_wall) {
        dest[dim_y]++;
      }
      break;
    case '1':
    ...
    case 'h':
    case KEY_LEFT:
if (dest[dim_x] != 1&& d->PC->position[dim_x]-3<dest[dim_x]&&d->PC->known_terrain[dest[dim_y]][dest[dim_x]-1]!=ter_wall) {
        dest[dim_x]--;
      }
      break;
    }
  ...
  refresh();
  count++;
  } while (c != 'p'&&c!='z'&&count!=2000);
  if(c=='z'&& d->PC->Mana>=10&&d->PC->cdS==0){
    do_combatS(d,d->PC,charpair(dest),dest);
    do_moves(d);
  }
 else if(charpair(dest)&&charpair(dest)->get_symbol()!='@'&&c=='p'){
      do_combatR(d,d->PC, charpair(dest));
      do_moves(d);
  }
    else{
  mvaddch(dest[dim_y] + 1, dest[dim_x], io_tochar(dest,d));
  refresh();
    }
}
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case'p':
      if(d->PC->equip[range]){
	if(d->PC->equip[range]->get_type()== objtype_RANGED){
